...
def test_blog_capture_retries_retryable_navigation_failure(tmp_path, monkeypatch) -> None:
    skill_dir = tmp_path / "skill"
    skill_dir.mkdir()
    bundle_dir = tmp_path / "bundle"
    sink_dir = tmp_path / "kb" / "inbox" / "openai" / "post"
    ...
    def fake_run_command(command, *, cwd: Path | None, timeout: int):
        ...
        if command[:3] == ["npx", "tsx", "scripts/run.ts"]:
            ...
            assert timeout == 180
            if (
                len([item for item in commands if item[:3] == ["npx", "tsx", "scripts/run.ts"]])
                == 1
            ):
                return CompletedProcess(
                    command,
                    1,
                    stderr="web capture failed: Navigation failed with HTTP 403",
                )
            return CompletedProcess(command, 0, stdout=f'{{"bundle_dir":"{bundle_dir}"}}')
        if command[:2] == ["clipsmith", "validate-bundle"]:
            return CompletedProcess(command, 0, stdout="{}")
    ...
    monkeypatch.setattr("alcove.blog_capture.shutil.which", lambda name: f"/bin/{name}")
    monkeypatch.setattr("alcove.blog_capture.time.sleep", lambda seconds: sleeps.append(seconds))
    ...
    assert sleeps == [2]
    assert [command[:3] for command in commands].count(["npx", "tsx", "scripts/run.ts"]) == 2
