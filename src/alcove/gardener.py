...
class GardenerModule:
    ...
    def scan(self) -> list[dict]:
                    ...
                    issues.append(
                        {
                            "kind": "empty_tag",
                            "path": str(doc.path),
                            "message": f"Tag {tag!r} has no references",
                        }
                    )
                elif count < 2:
                    issues.append(
                        {
                            "kind": "orphan_tag",
                            "path": str(doc.path),
                            "message": f"Tag {tag!r} is used only {count} time(s)",
                        }
                    )
        for topic, count in sorted(source_counts.items()):
            if count >= 3 and question_counts.get(topic, 0) == 0:
                issues.append(
                    {
                        "kind": "question_backlog",
                        "path": str(self.paths.knowledge / "topics"),
                        "message": f"Topic {topic!r} has {count} active sources but no Question docs",
                    }
                )
    ...
    def _stale_source(self, doc: MarkdownDoc) -> dict | None:
        if doc.path is None or doc.frontmatter.get("status", "active") != "active":
            return None
        published = str(
            doc.frontmatter.get("published_date") or doc.frontmatter.get("created_at") or ""
        )[:10]
